/* mood.js — Live Production (With Auto-Save) */
...
(() => {
  ...
  // --- Image Analysis (Auto-Save Added) ---
  async function handleImageAnalysis() {
    ...
    try {
      ...
      if (data.mood) {
        ...
        const detectedEmoji = MOODS.find(m => m.id === data.mood)?.emoji || "🤖";
        await saveMoodToAPI({
            mood: data.mood,
            emoji: detectedEmoji,
        ...
        // 3. Notify User
        if(analyzeStatus) analyzeStatus.textContent += " — Saved!";
      }
    } catch (error) {
      ...
      selectMood(randomMood.id, "image");

      // Auto-save the simulation too
      await saveMoodToAPI({
        mood: randomMood.id,
  ...
  async function saveMoodToAPI(entry) {
    try {
      ...
      loadMoods(); // Refresh the list immediately
